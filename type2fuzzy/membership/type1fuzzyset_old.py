...
class Type1FuzzySet:
    ...
    def __eq__(self, value):
        ...
        union_dom_len = len(
            list(set(self.degree_of_membership()).union(value.degree_of_membership())))
    ...
    def __str__(self):
        ...
        for domain_val, dom_val in self._elements.items():
            set_elements.append(
                f'{dec_places_formatter % dom_val}/{dec_places_formatter % domain_val}')
    ...
    @property
    def name(self)->str:
        '''returns the name of the fuzzy set'''
        return self._name
    ...
    def from_representation(cls, set_representation, name=''):
        ...
        if set_representation is None:
            raise Type1FuzzySetException('Type-1 Set Representation cannot be null')
        if set_representation is '':
            raise Type1FuzzySetException('Type-1 Set Representation cannot be empty')
        ...
        try:
            ...
            raise Type1FuzzySetException('Invalid set type-1 format') from exp
    ...
    def from_alphacut_type1_set(cls, alphacut_set, name=''):
        '''creates a type-1 fuzzy set from an alphacut set'''
        t1fs = cls()
    ...
    def create_triangular(cls, univ_low, univ_hi, univ_res, set_low, set_mid, set_hi, name=''):
        ...
        if univ_hi <= univ_low:
            raise Type1FuzzySetException('Error in universe definition')
        if (set_hi < set_mid) or (set_mid < set_low):
            raise Type1FuzzySetException('Error in triangular set definition')
        ...
        if idx_hi == idx_mid:
            ...
            for domain_val in domain_elements[idx_low:idx_hi+1]:
                dom = dom = max(
                    min(
                        (domain_val - set_low)/(set_mid - set_low),
                        (set_hi - domain_val)/(set_hi - set_mid))
                    , 0)
                t1fs.add_element(domain_val, round(dom, precision))
    ...
    def adjust_value(val, val_array):
        '''Adjusts the value to the nearest value in the array'''
        idx = (np.abs(val_array - val)).argmin()
        return val_array[idx]

    @classmethod
    def create_trapezoidal(cls, domain, a, b, c, d, name=''):
        '''creates a trapezoidal type 1 fuzzy set in a defined universe of discourse'''
        t1fs = cls()
    ...
    def add_element(self, domain_val, dom_val):
        ...
        if dom_val > 1:
            raise ValueError(
                f'degree of membership must not be greater than 1, {domain_val} : {dom_val}')
    ...
    def domain_limits(self):
        ...
        Returns:
        --------
        limits -- CrispSet containing the smallest an largest domain value
        '''
